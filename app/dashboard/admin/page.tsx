...
export default function TeacherDashboardPage() {
  ...
  useEffect(() => {
    ...
    const mySchedules = initialSchedules.filter(
      (s) => s.teacherId === parsed.id,
    );
    ...
    <div className="space-y-6 animate-fade-in text-slate-800 pb-12">
      ...
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">
          Selamat datang di Lumina LMS. Berikut ringkasan kelas hari ini, tugas
          siswa, dan statistik performa kelas Anda.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm flex flex-col justify-between hover:shadow-md transition-all">
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-800">
              {teacherSchedules.length} Sesi
            </h3>
            <p className="text-[11px] text-slate-400 font-medium mt-1">
              Jadwal mengajar aktif minggu ini
            </p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm flex flex-col justify-between hover:shadow-md transition-all">
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-800">
              {ungradedAssignments.length} Tugas
            </h3>
            <p className="text-[11px] text-slate-400 font-medium mt-1">
              Menunggu koreksi & penilaian
            </p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm flex flex-col justify-between hover:shadow-md transition-all">
          <div>
            ...
            <p className="text-[11px] text-slate-400 font-medium mt-1">
              Siswa aktif di semua kelas Anda
            </p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm flex flex-col justify-between hover:shadow-md transition-all">
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-800">
              {teacherCourses.length} Kelas
            </h3>
            <p className="text-[11px] text-slate-400 font-medium mt-1">
              Kelas materi ajar yang Anda ampu
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-3">
              {teacherSchedules.map((sch) => {
                ...
                return (
                  <div
                    key={sch.id}
                    className="flex justify-between items-center border border-slate-50 p-4 rounded-xl hover:border-slate-150 hover:bg-slate-50/30 transition-all"
                  >
                    <div className="flex items-center gap-3">
                      <span className="text-2xl p-1 bg-slate-50 border border-slate-100 rounded-lg">
                        {subject?.icon ?? "📚"}
                      </span>
                      <div>
            ...
            <div className="space-y-3">
              {ungradedAssignments.map((a) => {
                ...
                return (
                  <div
                    key={a.id}
                    className="p-3 bg-slate-50/50 border border-slate-100 rounded-xl space-y-1.5 hover:bg-slate-50 transition-colors"
                  >
